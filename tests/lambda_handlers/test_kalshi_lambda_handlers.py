...
from aws.lambdas.kalshi_markets import handler as legacy_markets_handler
...
from market_data_platform.lambda_handlers import kalshi_markets as markets_handler
...
class KalshiLambdaHandlerTests(unittest.TestCase):
    ...
    def test_markets_lambda_handler_dispatches_to_pipeline(self):
        context = Mock()

        with patch.object(markets_handler, "run", return_value={"row_counts": {"markets": 1}}) as mock_run:
            result = markets_handler.lambda_handler({"market_ticker": "KXTEST"}, context)

        self.assertEqual(result, {"row_counts": {"markets": 1}})
        mock_run.assert_called_once_with({"market_ticker": "KXTEST"})
    ...
    def test_handlers_use_empty_event_for_none(self):
        ...
        with patch.object(markets_handler, "run", return_value={"row_counts": {}}) as mock_markets_run:
            markets_handler.lambda_handler(None, Mock())

        mock_events_run.assert_called_once_with({})
        mock_series_run.assert_called_once_with({})
        mock_markets_run.assert_called_once_with({})
    ...
    def test_legacy_handler_modules_delegate_to_packaged_entrypoints(self):
        ...
        self.assertIs(legacy_markets_handler.run, markets_handler.run)
        ...
        with patch.object(legacy_markets_handler, "run", return_value={"row_counts": {"markets": 5}}) as mock_markets_run:
            result = legacy_markets_handler.lambda_handler({"market_ticker": "KXTEST"}, Mock())

        self.assertEqual(result, {"row_counts": {"markets": 5}})
        mock_markets_run.assert_called_once_with({"market_ticker": "KXTEST"})
